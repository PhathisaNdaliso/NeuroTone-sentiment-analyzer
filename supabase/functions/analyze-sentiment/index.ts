...
async function fetchWith429Retries(input: RequestInfo | URL, init: RequestInit) {
  const maxRetries = 2;
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    const backoffMs =
      Number.isFinite(retryAfterSeconds) && retryAfterSeconds > 0
        ...
        : Math.min(12_000, baseDelayMs * 2 ** attempt) + jitterMs;

    console.error(`AI gateway rate limit (429). Retry ${attempt + 1}/${maxRetries} in ${backoffMs}ms`);
...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt =
      "You are an expert sentiment analysis system. Analyze the given text accurately, considering context, sarcasm, and implicit meanings.";

    const response = await fetchWith429Retries("https://ai.gateway.lovable.dev/v1/chat/completions", {
      method: "POST",
      headers: {
                    ...
                    items: {
                      ...
                      properties: {
                        ...
                        percentageContribution: {
                          type: "number",
                          description: "Percentage contribution 0-100",
                        },
                      },
